...
//    }
    ...
    @Override
    public List<UserExtra> findProfileByOrganizationId(List<Organization> ids){
//        return this.profilRepository.findByOrganization(id);
        ...
        List<UserExtra> filteredUsers = userExtraList.stream()
                .filter(user -> listOrganizationIds.contains(user.getOrganization().getId()))
...
//        return this.profilRepository.findByOrganizationIdIn(listOrganizationIds);
    ...
    @Override
    public List<UserMinimal> getUsersByOrgs(List<ObjectId> objectIdList) {
        return userExtraRepository.findAllByOrganizationIds(objectIdList);
    }
